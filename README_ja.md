...
*2つのtmuxセッションでAIエージェント11体を起動 — 将軍1体 + 後方10体を **Claude Code / OpenAI Codex / GitHub Copilot / Kimi Code / OpenCode / Cursor / Antigravity** で編成*
...
<p align="center"><i>家老1体が足軽7体・軍師1体・大目付1体へルーティング — 実際の稼働画面、モックデータなし</i></p>
...
将軍が委譲 → 家老がルーティング → 足軽7体が実行 → 軍師が証跡/QCをレビュー → 家老が受入判定。
大目付は、家老から割り当てられた対象または最終成果物を独立レビューします。
あとはダッシュボードを眺めるだけ。
...
**なぜ使うのか？**
- 1コマンドで、将軍・家老・足軽7体・軍師・大目付の合計11体を起動
...
```
    ...
    └──────┬──────┘
           │ YAMLファイル + tmux
    ┌──────▼──────┐
    │    KARO     │  ← ルーティング・dashboard更新・受入判定の唯一の担当
    └──────┬──────┘
           │
  ┌─┬─┬─┬─┴─┬─┬─┬────────┬────────────┐
  │1│2│3│4│5│6│7│ GUNSHI │ OOMETSUKE  │
  └─┴─┴─┴─┴─┴─┴─┴────────┴────────────┘
     ASHIGARU      QC/RCA    独立レビュー
```
...
| **並列性** | 逐次実行（1つずつ） | 複数の独立セッション | 並列ノード（v0.2+） | 限定的 | **足軽7体の並列実行 + 独立レビュー役** |
...
**連携コストゼロ** — エージェント間の通信はディスク上のYAMLファイル。モデル呼び出しは実作業にのみ使われ、後方10体の連携自体はYAMLとtmuxで行います。

**完全な透明性** — すべてのエージェントが見えるtmuxペインで動作。すべての指示・報告・判断がプレーンなYAMLファイルで、読んで、diffして、バージョン管理できます。ブラックボックスなし。

**実戦で鍛えた階層構造** — 命令は殿→将軍→家老、作業証跡は足軽→軍師→家老へ流れ、大目付は対象/最終レビューを家老へ助言します。ルーティング、`dashboard.md` 更新、受入/差戻しは家老だけが行います。

---

## なぜCLI（APIではなく）？

API利用料は稼働エージェントごとに増えます。CLI定額サブスクは多体編成の予測可能性を高めますが、提供条件と価格は変わるため、必ず現在のprovider情報で確認してください：

| | API（従量課金） | CLI（定額制） |
|---|---|---|
| **全編成** | モデル呼び出しごとに増加 | 現在のprovider契約に依存 |
| **コスト予測性** | 予測不能なスパイク | 月額固定 |
| **使用時の心理** | 1トークンが気になる | 使い放題 |
| **実験の余地** | 制約あり | 自由に投入 |

**「AIを使い倒す」思想** — 成果や待ち時間を改善する場面では足軽7体と専門レビュー役を投入しつつ、同梱の古い価格表ではなく現在のprovider制限を検証します。
...
OpenCode の起動は `--agent` で生成済み `.opencode/agents/<agent_id>.md` を読み込み、リセットは `/new`、モデル変更は再起動で行う。生成frontmatterには、将軍は委譲、家老だけがルーティング/dashboard更新/受入判定、足軽は割当済みtask/reportのみ、軍師はQC/RCA、大目付は家老向け独立レビュー、という境界を埋め込みます。
...
足軽がタスクを実行する中で、**再利用可能なパターンを発見**し、軍師を経由して家老へ報告します。家老だけが候補を `dashboard.md` に集約し、殿（あなた）が取り込み審査を始めるか判断します。
...
気づき: 「このパターン、3つのプロジェクトで同じことをした」
    ↓
足軽→軍師→家老のYAML報告:  skill_candidate:
                 found: true
                 name: "api-endpoint-scaffold"
                 reason: "3プロジェクトで同じRESTスキャフォールドパターンを使用"
    ↓
dashboard.md に掲載 → 殿が「このスキル追加」と依頼 → source/license/risk審査
    ↓
Shogun採用分はGitレビュー済みSkill Registryへ。個人実験はunmanagedのまま
```

スキルは実作業から成長しますが、発見は配備ではありません。Registry管理スキルはコミットされ再現可能、unmanagedな個人実験はリポジトリや別システムへ自動コピーしません。
...
Androidアプリでは将軍ターミナルと主要9ペインを利用できます。全runtimeは11体になったため、大目付ペインはアプリ側の対応が入るまでtmuxから開いてください。
...
| 📋 家老（Karo） | 交通整理 - ルーティング・dashboard更新・受入判定の唯一の担当。確認は機械的検査のみ | 1 |
| ⚔️ 足軽（Ashigaru） | ワーカー - 実装タスクを並列実行 | 7 |
| 🧠 軍師（Gunshi） | 品質参謀 - QC・証跡レビュー・RCA | 1 |
| 🔍 大目付（Oometsuke） | 独立レビュー - 対象/最終レビューを家老へ助言 | 1 |

tmuxセッションが作成されます：
- `shogun` - ここに接続してコマンドを出す
- `multiagent` - 家老・足軽7体・軍師・大目付の10ペインがバックグラウンドで稼働
...
| カスタムスキル | `~/.claude/skills/`, `~/.agents/skills/`, `skills/` | 関連 trigger 発火時 |
...
5. 将軍がメッセージを読み、家老のルーティングと足軽→軍師→家老の証跡フローで処理
...
- 家老がタスク割当・完了時に自動更新
- 後方10ペインを見れば、ルーティング・実行・QC・レビュー状況を確認できる
...
実効CLI・モデル識別子・Thinking設定は、現在の `config/settings.yaml` とインストール済みCLIが実際に提供する機能で決まります。このREADMEをモデル提供状況や価格の正本として扱わないでください。

| エージェント | 運用上の責務 |
|-------------|-------------|
| 将軍 | 殿の命令を家老へ委譲し、重要な判断を殿へ提示 |
| 家老 | ルーティング・`dashboard.md` 更新・受入判定の唯一の担当。前提/manifestの機械的確認だけを行う |
| 足軽1-7 | 家老が割り当てた実装・調査・テスト・deployを実行 |
| 軍師 | 定性的QC、証跡レビュー、能力評価、RCAを担当 |
| 大目付 | 対象/最終成果物を独立レビューし、家老へ助言 |

**Thinking制御**: `config/settings.yaml` でエージェントごとに `thinking: true/false` を設定します。adapterが選択CLI向けに変換し、ペインボーダーへ設定上の表示名を出します。表示名だけで実際の対応可否を推測せず、現在のCLIで検証してください。

**現在のモデル運用**:

- `shogun-model-list` はcanonical設定とインストール済みCLI/公式文書から、検証時刻付きの読み取り専用inventoryを作ります。不明な項目は `unverified` のままにします。
- `shogun-bloom-config` はレビュー用提案だけを作ります。軍師が能力/fallbackを評価し、ルーティング状態を受け入れるのは家老だけです。provider・費用・権限の重要変更は大目付へ回します。
- `shogun-model-switch` はRegistry収録12件の1件ですが、現在は `quarantined` で配備しません。未整備なのはtransactionalかつ権限を拡張しないSkill adapterです。既存のoperator管理core切替コマンドはRegistry Skillとは別境界で、このquarantineでは変更しません。agent起動を有効化するには、安全なadapter・レビュー・明示承認が必要です。

**認知的複雑さ**でのルーティングは2段階です。**エージェントルーティング**では足軽が境界付き実行、軍師が定性分析/QC/RCAを担当し、必要な場合だけレビュー済み `capability_tiers` で足軽内のモデルを分けます。
...
家老が各サブタスクにBloomレベルを付け、唯一の担当としてルーティングします。L1-L3の実行は原則足軽、定性的なL4-L6分析とすべてのQC/RCAは軍師へ回します。境界付きの機械的確認は足軽へ割り当てられますが、家老が定性レビューを引き取ることはありません。
...
エージェント単位のルーティングに加え、**足軽階層内でのモデルレベルルーティング**も設定できます。`capability_tiers` には現在のhostで検証済みの識別子だけを使います。次は形式例であり、提供中モデルの主張ではありません：
...
利用不能時の結果/fallbackを明記し、より高価または信頼境界の異なるproviderへ黙って置換しないでください。

設定を支援する2つの組み込みスキルがあります：

| スキル | 用途 |
|--------|------|
| `shogun-model-list` | 検証元/時刻と `unverified` を含む最新の読み取り専用inventory |
| `shogun-bloom-config` | 現在検証できるモデル・制約・fallbackに基づく役割安全な提案 |

現在の制約を集めてから、将軍へ `shogun-bloom-config` を依頼してください。軍師のレビューと家老の受入が終わるまで、出力YAMLは提案です。
...
2. **ルーティング責任者を1体に限定**: 家老だけが分解・割当・依存管理・`dashboard.md` 更新を行う
3. **証跡フロー**: 足軽が実行して軍師へ報告し、軍師がQC/RCAを担当して家老へ報告する
4. **独立レビュー**: 大目付は対象/最終checkpointで家老へ助言するが、ルーティング権限は持たない
5. **受入を明確化**: 家老は機械的な完備確認を行い、受入/差戻し/再割当だけを記録する
6. **人間への窓口を一元化**: 重要な判断を殿へ伝えるのは将軍だけ
...
2. **情報集約**: 家老は軍師/大目付の結論と、そこから参照される足軽証跡を受け取る
3. **状態の一貫性**: ルーティング・機械的完了状態・受入判断の所有者を1体に固定
...
リポジトリには、レビュー済みの小さな必須セットを同梱します。追加スキルは運用中に発見し、Skill Registryの審査を通したものだけを採用します。

将軍へスキル名を指定してください。同じportable正本から、各CLIが対応するメタデータだけを生成します。

### Registryスキル（リポジトリにコミット済み）

Git-canonicalなRegistryは `skills/` にレビュー済み12件を収録し、現在は11件を配備対象、1件をquarantineとしています：

| スキル | Registry状態 | 説明 |
|--------|--------------|------|
| `skill-creator` | enabled | portableかつ役割安全なスキルと圧力テストを設計 |
| `shogun-skill-intake` | enabled | 「このスキル追加」をCodex AppとShogun Registryに分けて処理 |
| `shogun-agent-status` | enabled | 境界付き・読み取り専用の稼働要約を作成 |
| `shogun-model-list` | enabled | 古い価格表に依存せず、現在のモデルを検証して一覧化 |
| `shogun-bloom-config` | enabled | 現在確認できる提供条件から役割安全なBloom割当案を作成 |
| `shogun-model-switch` | **quarantined** | 配備しない: 安全なtransactional Registry adapterが未承認。core operatorコマンドは別境界 |
| `shogun-readme-sync` | enabled | README.mdとREADME_ja.mdの構造・事実を同期 |
| `shogun-screenshot` | enabled | 必要最小限の画面証拠を取得・秘匿化。crop/maskには任意依存のPillowが必要 |
| `shogun-systematic-debugging` | enabled | 修正前に根本原因を証拠で確定 |
| `shogun-test-first` | enabled | 観測済みRED-GREEN-REFACTORを必須化 |
| `shogun-verification-before-done` | enabled | 現在revisionの新鮮な完了証拠を必須化 |
| `shogun-review-response` | enabled | レビュー指摘を評価してから実装 |

`first_setup.sh` はコミット済みlockを検証し、enabledの11件を `~/.claude/skills/` と `~/.agents/skills/` の両方へコピーします。配布時にlockを再生成しません。**apply後はClaude/Codex CLIを新しいセッションで起動してください。** 稼働中セッションは追加スキルを再読込しません。

Windows Codex AppとWSL2 ShogunはGit境界で分離し、設定・認証・session・Drive領域を共有しません。同じimmutableなmerge commitを基準に各側を独立インストール/検証し、live directory同士を同期しないでください。検証・rollback・復旧・provenance・2システム間の取り込み契約は [Shogun Skill Registry](docs/skill-registry.md) を参照してください。

### 期待できる改善例

- **不具合修正:** 最初に見えた症状へ即パッチせず、足軽が再現し、軍師が根本原因を確認し、家老が証拠付きの修正だけを受け入れます。
- **機能開発:** 観測済みの失敗テストを先に作り、最小実装で通してから整理するため、回帰を早く検出できます。
- **完了・レビュー:** 「完了」は現在revisionで実行した件数付き結果を必須とし、レビュー指摘はコード変更前に採用・却下・要確認へ評価します。
- **CLI間の一貫性:** ClaudeとCodexは同じレビュー済み動作を参照し、各CLIが対応するmetadataだけを保持します。

### スキルの思想

**1. Registry管理スキルと個人スキルはlifecycleを分ける**

承認済み共有スキルは `skills/` にコミットし、`skills/registry.yaml` で宣言し、`skills/registry.lock.yaml` で固定します。`.claude/commands/` などユーザ所有場所の個人実験はunmanaged・未コミットのままです：

- intake、provenance、license、risk review、testなしに個人スキルをRegistryへコピーしない。
- Windows Codex Appへ入ったスキルをWSL2 Shogunでも自動承認せず、毎回 `adapted` / `codex-only` / `excluded` / `pending` を提案する。
...
殿（あなた）が内容を確認
    ↓
「このスキル追加」と依頼し、immutable sourceとlicenseを審査
    ↓
Codex Appへの導入を別途検証し、Shogunでの扱いを提案
    ↓
Shogun採用を承認した場合だけGitレビュー後・merge後に反映
```

`codex-only` または `excluded` が妥当な場合は理由を説明し、殿の承認を求めます。スキル追加はユーザ主導であり、この文言は審査開始を意味し、自動的な信頼を意味しません。
...
alias csm='tmux attach-session -t multiagent'  # 家老・足軽7体・軍師・大目付へ接続
```
...
│   │   ├── karo.yaml         # 家老へのメッセージ
│   │   ├── ashigaru{1-7}.yaml # 各足軽へのメッセージ
│   │   ├── gunshi.yaml       # 軍師へのメッセージ
│   │   └── oometsuke.yaml    # 大目付へのメッセージ
│   ├── tasks/                # 各ワーカーのタスクファイル
│   └── reports/              # ワーカーレポート
│
...
│
├── skills/                   # Git-canonicalなportableスキル正本
│   ├── registry.yaml         # lifecycle 12件（enabled 11 / quarantined 1）
│   └── registry.lock.yaml    # deterministicなsource/render lock
│
...
> **管理職を作業列に入れない。** 家老はワークフロー進行・委譲・最終受付に集中します。E2E計画/証跡レビューとRCAは軍師、実行は足軽へ回し、家老は機械的確認だけを行います。
...
- **E2E責任を再定義** — 軍師がE2E計画/証跡をレビューし、足軽が実行し、家老は機械的な前提/manifest確認後に最終pass/failを受け入れる
...
- **OpenCodeエージェント生成** — `scripts/build_instructions.sh` が、他CLIと同じ共通指示ソースから `.opencode/agents/*.md` を将軍/家老/足軽1-7/軍師/大目付向けに生成
