...
`workspace/codex/CODEX_DESKTOP_STARTUP.md`とShogunの`docs/github-boundary-operation.md`へ、固定snapshotがallowlist metadataと固定log集計をlocal process内で取得することだけを限定例外として記載する。
...
7. Workspace専用branchで共通起動手順へ限定例外を追加する。
8. Workspace PRをmainへ反映する。
9. §5.3の全tokenを含む位置までのprefix許可を設定する。
10. Codexから固定commandを実行し、raw本文と秘密値が出力されないことを確認する。
