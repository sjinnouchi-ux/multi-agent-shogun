...
Workspace main反映後、実PCの`C:\Users\jinnouchi\.codex\AGENTS.md`を`CODEX_DESKTOP_CUSTOM_INSTRUCTIONS.md`の`Paste This Text`と一致させる。これはGit管理成果物ではなくhost設定であり、別PCやShogunへコピーしない。
...
配置時はmode `0555`とし、source repo、source commit、SHA-256、配置日時を本文なしmanifestへ記録する。`sudo`とsystem directoryは使用しない。snapshot更新は通常診断とは別の明示的deployment作業とする。

manifest path:

```text
/home/jinnouchi/.local/share/shogun-codex-diagnostics/manifest.json
```

manifestはmode `0444`とし、秘密値、runtime path一覧、診断結果を保存しない。
...
| tool | object | version=`1.0.0`、deployment=`user_local_snapshot` |
...
```json
{"schema_version":1,"generated_at":null,"ok":false,"overall":"unavailable","tool":{"version":"1.0.0","deployment":"user_local_snapshot"},"repository":null,"sessions":[],"processes":null,"global_sources":{},"agents":[],"errors":[{"code":"internal_error","component":"diagnostic","agent":null}],"warnings":[]}
```
...
`diagnostic_process_failed`はconsumerが合成するcodeであり、正常に起動したCLI自身は出力しない。optional source missingはerrors/warningsのどちらにも追加せず、source objectの`state=missing`だけで示す。required sourceの`source_rejected`はerrors、optional sourceの`source_rejected`はwarningsへ入れるがcodeは同じとする。
...
- raw bytes由来のhashやfingerprintを出力しない。
- parse error、command error、path errorは固定codeだけへ変換する。

### 8.5 snapshotと残余リスク

作業repo内sourceを直接恒常許可しない。main反映済みsourceを確認後、repo外user-local pathへsnapshot配置し、mode `0555`とmanifestで偶発的変更を検出しやすくする。
...
`workspace/codex/CODEX_DESKTOP_STARTUP.md`、`workspace/codex/CODEX_DESKTOP_CUSTOM_INSTRUCTIONS.md`、Shogunの`docs/github-boundary-operation.md`へ、固定snapshotがallowlist metadataと固定log集計をlocal process内で取得することだけを限定例外として記載する。
...
- remote名・URL、file名、PID、command line、正確なfile size、raw hashが出力されない。
...
production CLIにはsession/socket overrideを追加しない。testはunique tmux socket上に固定名`shogun`と`multiagent`を作り、collectorのcommand runnerをtest codeからだけ注入する。
...
5. main上sourceのSHA-256を確認し、user-local snapshotとmanifestを配置する。
6. snapshotのhash、mode、固定commandのJSON契約を確認する。
7. Workspace専用branchで共通起動手順、カスタム指示文面、作業ログへ限定例外を追加する。
8. Workspace PRをmainへ反映する。
9. 実PCのグローバル`AGENTS.md`をmain反映済みカスタム指示文面と一致させる。
10. §5.3の全tokenを含む位置までのprefix許可を設定する。
11. 新しいCodex taskと現在taskの両方から固定commandを実行し、raw本文と秘密値が出力されないことを確認する。

Workspace規則を先に緩めない。Shogun main反映、snapshot配置、契約test完了後にだけ例外を有効化する。
...
10. Workspace main、カスタム指示文面、実PCグローバル`AGENTS.md`の限定例外が一致する。
