...
常時許可される診断production codeは`scripts/codex_diagnostics.py`一fileだが、次の関数境界を維持する。
...
Canonical `shutsujin_departure.sh`はagent watcherを直接起動し、
`watcher_supervisor.sh`を管理processとして起動しない。したがってsupervisorは
optional observationであり、count `0` / state `missing`はnot-managedとして記録するが、
errorにも健康度低下にも使わない。`duplicate`と`unknown`はprocess境界を確定できない
ため、引き続き`degraded`とする。supervisorを将来必須化する場合はlauncher/control-plane
を変更する別の明示承認taskとし、本v1へ暗黙に含めない。
...
完全schemaはfield単体の型だけでなく、次のcross-field invariantも含む。

- 成功documentの`canonical_remote_present`は常に`true`。
- session `missing`は3 countがすべて`0`、`error`はすべて`null`、`present`は
  bounded integerでpane countが1以上かつdead/unknown count以下にならない。
- 未観測agentは`session=null`、`pane_state=not_observed`、`cli=unknown`、
  watcher count/state=`null/not_observed`。観測agentのsession、pane、watcher count/state、
  source applicabilityは§6の配置と対応する。
- source `present`だけがtimestampとsize classを持ち、`missing|rejected|error`は両方
  `null`。`not_applicable`はapplicability/stateの両方が`not_applicable`である。
- supervisorとagent watcherのcount/state pair、session pane countとagent配置、
  errors/warnings severityを相互検証する。
- `log_events`はtimestampと全countが揃った完全値、または全て`null`のどちらかにする。
  完全値に同一agentのlog issueがあればschema不正とする。全`null`のobserved agentは
  `required_source_missing | source_rejected | command_failed`のいずれか1件を
  `errors`の`component=log`、同一agentへ持つ。全`null`の未観測agentは通常のmissingなら
  issueなし、rejected/I/O失敗なら`source_rejected | command_failed`のいずれか1件を
  `warnings`の`component=log`、同一agentへ持つ。global log issue、誤severity、誤code、
  複数issueは拒否する。§7.1の実際に上限へ達した配列だけは欠落issueを許容するが、
  保持された矛盾はtruncation時も拒否する。
- `overall`はrepository availability、session、errors、process、required sourceから
  validatorが再計算した値と完全一致し、自己申告値を単独では信頼しない。
...
`result_truncated`のcomponent/agentは`diagnostic/null`であり、errorsまたはwarningsの
少なくとも一方が上限64件でなければschema不正とする。errorsが64件の場合だけ欠落した
error相関、warningsが64件の場合だけ欠落したwarning相関を許容する。保持されている
矛盾issueはtruncation時も拒否し、短い配列へmarkerだけを足したdocumentは信頼しない。
...
```text
command_failed, source_rejected, unknown_agent_observed, unknown_cli_observed
```
...
全code enum、CLI errors、CLI warnings、consumer decision-only codeは別集合として
production/consumerの双方で検証する。`command_failed`はrequired/collector failureでは
error、optional source/log I/O failureではwarningになり得る。warning-only codeをerrorsへ、
error-only codeをwarningsへ、consumer-only codeをCLI JSONへ入れたdocumentはschema不正である。

### 7.4 overall算出

- `unavailable`: `shogun`と`multiagent`が両方missing、またはrepository collectorが利用不能
- `degraded`: sessionが一方だけmissing、present sessionのdead pane、agent session不一致、supervisorの`duplicate|unknown`、command queue異常、観測済みagentのwatcher異常、観測済みagentのrequired source異常、collector errorのいずれか
- `healthy`: 上記以外

未観測agentのsource欠落、optional source欠落、supervisor `missing`、Shogun停止時の
stale runtime fileは健康度を悪化させない。
...
- timeout/output limit後はprocess groupへSIGKILLし、全体deadlineとは別の0.1秒以下の
  bounded reap graceで`wait`する。grace超過時はpollだけを行い、無制限waitしない。
...
rollback maintenanceでは、snapshot parentのpinned FDがeffective user所有かつgroup/world write不可であり、visible fixed snapshot leafが同じpinned inode、effective user所有、regular file、mode `0555`、期待hashであることを開始時とcommit境界で検証する。replaceがold分類の例外を返した場合は、exact temporary inodeのunlinkとdirectory fsyncを完了した後に同じparent binding/trustと旧leafのinode/owner/mode/hashを再検証できた場合だけexit 3を許す。rollback中は協調する同一UIDのmaintenance writerをすべて停止する。同一UIDの悪意ある非協調writerが最終検証後にもraceを継続するケースは§8.5どおりv1の対象外であり、このhelperを同一UID攻撃者に対するsandboxとは扱わない。
...
host `AGENTS.md`への初回挿入とrollback時削除は、candidate作成時の`Before` bytesを、`FileMode.Open`・`FileAccess.ReadWrite`・`FileShare.None`・`WriteThrough`で一度だけ開いた同一host `FileStream`から再読したbytesと完全比較するCASとする。staleならbackup作成前にno-op停止する。host write前にtask-local backupを`CreateNew`・`FileShare.None`・`WriteThrough`で作り、`Flush(true)`とsame-stream readbackを完了する。backup `FileShare.None` handleはcandidate write・検証・必要なrestore判定が終わるまで開いたままにし、verified commitの場合だけclose後に削除する。candidateのwrite、`SetLength`、`Flush(true)`、readback、marker検証は同じexclusive host handleだけで行う。writeまたは検証失敗時は同じhandleでverified backupを復元してdurable flush/readback/marker検証し、restore成功・不確定のどちらでもbackupとcandidateを保持して、初回導入ではcommand許可前、rollbackでは後続変更前に停止する。BOM、LF/CRLF、marker外bytesは完全保持し、別openによるpost-check、file copy/move、stale local backupへの置換は禁止する。Task 12のmarker削除は依存helperを含む同一のmarker付きPowerShell blockをverbatimで持ち、具体的なreverse byte-delta検証後にbefore marker `1/1`、candidate marker `0/0`として使う。

### 8.7 consumerとrollback maintenance境界

Codex consumerは各実行についてGitHub raw registryを先に検証し、marker順序・cardinality、全record exact keys、唯一のactive recordを満たさない通常例外をすべて`diagnostic_provenance_untrusted`へ変換する。commandはexit 0、10秒未満、stderr空、ASCII JSON一件を必須とする。JSONはtop-levelだけでなくrepository、sessions 2件、processes、global sources 2件、agents 11件、各source/log event、errors/warningsのexact keys、順序、enum、count上限までproduction `validate_document()`と同じ完全schemaを独立に検証する。nested free text、literal non-ASCII、cardinality違反、nonzero exit、parse時例外は`diagnostic_process_failed`とし、いずれもraw fallbackを許さない。

rollback helperはfixed snapshotだけを対象にし、`O_NOFOLLOW`が無ければ拒否する。snapshot parentはeffective user所有かつgroup/world write不可、current leafはeffective user所有・regular file・mode `0555`・failing active hash一致を開始時とcommit境界で要求する。target Git blobが明示選択されたsuperseded record hashと一致し、両hashが異なる場合だけ、同一directoryのeffective user所有・mode `0555` temporary fileをfsyncして`os.replace`する。replace前の拒否でもexact temporary inodeのcleanup/unlink/directory fsyncを証明できない場合があるため、exit 3はcleanup/durability完了後にparent binding/trustとvisible旧leafのpinned inode/owner/mode/hashを再検証できた拒否だけに使う。snapshot commit状態、またはtemporary artifactのcleanup/durability状態のどちらかを確定できない場合は`RollbackCommitOrCleanupIndeterminate`・exit 4とし、外部hashを`target|failing|other|unreadable`の固定enumへ分類しても自動継続せず新しい明示recovery taskを要求する。exit 0だけが検証済みcommitであり、その後のdeployment record PRへ進める。
...
- consumer契約testはGitHub取得不能、marker欠落・重複・逆順・deep nesting、registry schema不正、active 0件・複数、hash不一致をすべて`diagnostic_provenance_untrusted`へ変換し、raw fallbackしないことを確認する。
- consumerはproductionと同じ完全nested schema、ASCII、sessions 2件、agents 11件、exact keys/order/enum/count caps、exit 0、stderr空、10秒未満を要求する。nested free text、cardinality違反、literal non-ASCII、exit 2/3、空・partial・複数JSON、parse exceptionを`diagnostic_process_failed`へ変換する。
- production/consumerは独立実装でcross-field invariantと`overall`再計算を検証する。
  overall/errors矛盾、Canonical remote false、session state/count矛盾、source
  applicability/state矛盾、agent observed/session/pane/watcher矛盾、issue severity違反、
  log event/issue矛盾、truncation配列内に保持された矛盾、CLI中のconsumer-only codeを
  hostile fixtureで拒否する。
...
rollbackはこの通常導入順序の自動分岐ではない。必要時は、(1) full-command許可撤回、(2) host markerのblock外raw bytes保持と同一exclusive handle CAS・durable backup・verified restoreを伴う削除、(3) Workspace policy revert PRとraw main確認、(4) active/superseded recordの明示選択とGit blob hash照合、(5) tested helperによるatomic replacement、(6) exit 0の場合だけsole active recordを更新するShogun work-log PR、の順とする。host marker transactionがstale、失敗、またはrestore indeterminateなら許可を撤回したまま(3)以降へ進まない。snapshot helperのexit 4は外部hashを照合しても自動継続せず、recordを更新しない。
