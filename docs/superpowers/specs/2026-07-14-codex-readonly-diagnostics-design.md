...
rollback helperはfixed snapshotだけを対象にし、`O_NOFOLLOW`が無ければ拒否する。current bytesがfailing active hash、target Git blobが明示選択されたsuperseded record hashと一致し、両hashが異なる場合だけ、同一directoryのmode `0555` temporary fileをfsyncして`os.replace`する。replace前の拒否はexit 3で旧snapshot不変とする。`os.replace`後のdirectory fsyncまたはpost-hash検証失敗は旧状態と同一視せず`RollbackCommittedIndeterminate`・exit 4とし、外部hashを`target|failing|other|unreadable`の固定enumへ分類しても自動継続せず新しい明示recovery taskを要求する。exit 0だけが検証済みcommitであり、その後のdeployment record PRへ進める。
...
- replace後のdirectory fsync/post-read失敗はpre-commit失敗と区別してexit 4となり、外部hash reconciliationと停止を要求する。
...
rollbackはこの通常導入順序の自動分岐ではない。必要時は、(1) full-command許可撤回、(2) host markerのblock外raw bytes保持を伴う削除、(3) Workspace policy revert PRとraw main確認、(4) active/superseded recordの明示選択とGit blob hash照合、(5) tested helperによるatomic replacement、(6) exit 0の場合だけsole active recordを更新するShogun work-log PR、の順とする。exit 4は外部hashを照合しても自動継続せず、recordを更新しない。
...
13. rollback testがpre-commit不変とpost-commit indeterminate exit 4を区別し、helperが通常診断・恒常許可へ入らない。
