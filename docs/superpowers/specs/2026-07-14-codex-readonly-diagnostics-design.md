...
rollback helperはfixed snapshotだけを対象にし、`O_NOFOLLOW`が無ければ拒否する。current bytesがfailing active hash、target Git blobが明示選択されたsuperseded record hashと一致し、両hashが異なる場合だけ、同一directoryのmode `0555` temporary fileをfsyncして`os.replace`する。replace前の拒否でもexact temporary inodeのcleanup/unlink/directory fsyncを証明できない場合があるため、exit 3は旧snapshot不変かつcleanup完了を検証できた拒否だけに使う。snapshot commit状態、またはtemporary artifactのcleanup/durability状態のどちらかを確定できない場合は`RollbackCommitOrCleanupIndeterminate`・exit 4とし、外部hashを`target|failing|other|unreadable`の固定enumへ分類しても自動継続せず新しい明示recovery taskを要求する。exit 0だけが検証済みcommitであり、その後のdeployment record PRへ進める。
